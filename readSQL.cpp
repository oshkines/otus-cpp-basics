...
std::vector<Record> readSql(int colRecords, string code = "") {
    ...
    if (code == "")
    {
        baseRequest = "SELECT\
            ...
            CASE WHEN _Reference541._fld55051 THEN 'Yes' ELSE 'No' END AS Klient,\
            CASE WHEN _Reference541._fld55053 THEN 'Yes' ELSE 'No' END AS Postavshik,\
            ...
            WHERE _marked = false\
            GROUP BY Code, Name, Klient, Postavshik, birthdate, TypeKI, KontactInformation\
            ORDER BY _code\
            LIMIT $1";
